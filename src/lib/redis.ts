...
// Lazy singleton connection, backing the live-chat feature. Cached on
// globalThis so we only ever open one connection per serverless instance
// (and per Node process during `next dev`, where hot-reload would
// otherwise leak a new connection on every file save) - the same pattern
// commonly used for a Prisma client singleton, for the same reason.
...
function buildClient(): RedisClientType {
  ...
  if (!url) {
    throw new Error(
      "REDIS_URL is not set. Add the Vercel Redis integration (or any redis:// connection string) to your environment variables before using live chat."
      );
  }
